...
class AudioManager:
    ...
    def __init__(self, config_manager=None):
        ...
        if self.config_manager:
            ...
            self.start_volume = self.config_manager.get_setting(
                "start_sound_volume", 0.3
            )  # Default 30% volume
            self.stop_volume = self.config_manager.get_setting(
                "stop_sound_volume", 0.3
            )  # Default 30% volume
            self.start_sound_path = self.config_manager.get_setting(
                "start_sound_path", None
            )  # Custom start sound path
            self.stop_sound_path = self.config_manager.get_setting(
                "stop_sound_path", None
            )  # Custom stop sound path
        else:
        ...
        self.volume = 0.3  # Default volume for _play_sound method
        ...
        if not self.start_sound_available or not self.stop_sound_available:
            ...
            if self.start_sound_path or self.stop_sound_path:
                print(
                    f"   Custom paths specified: start='{self.start_sound_path}', stop='{self.stop_sound_path}'"
                )
    ...
    def _is_ffplay_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "ffplay"], capture_output=True, text=True, timeout=5
            )
    ...
    def _is_aplay_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "aplay"], capture_output=True, text=True, timeout=5
            )
            return result.returncode == 0
        except Exception:
            return False
    ...
    def _is_paplay_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "paplay"], capture_output=True, text=True, timeout=5
            )
            return result.returncode == 0
        except Exception:
            return False
    ...
    def _play_with_ffplay(self, sound_file: Path, volume: float) -> bool:
        ...
        try:
            ...
            def play_audio() -> None:
                # Pass environment variables to subprocess for audio connection
                env = os.environ.copy()
                try:
                    subprocess.run(cmd, capture_output=True, timeout=10, env=env)
                except subprocess.TimeoutExpired:
                    # Silently handle timeout - audio may still play
                    pass
                except Exception as e:
    ...
    def play_start_sound(self) -> bool:
        ...
        print(
            f"🔊 play_start_sound called - enabled: {self.enabled}, available: {self.start_sound_available}"
        )
        if not self.enabled or not self.start_sound_available:
            print(
                f"❌ Start sound blocked - enabled: {self.enabled}, available: {self.start_sound_available}"
            )
            return False

        print(
            f"🔊 Playing start sound: {self.start_sound} (volume: {self.start_volume})"
        )
    ...
    def play_stop_sound(self) -> bool:
        ...
        print(
            f"🔊 play_stop_sound called - enabled: {self.enabled}, available: {self.stop_sound_available}"
        )
        if not self.enabled or not self.stop_sound_available:
            print(
                f"❌ Stop sound blocked - enabled: {self.enabled}, available: {self.stop_sound_available}"
            )
            return False
