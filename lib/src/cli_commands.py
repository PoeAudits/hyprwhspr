...
try:
    ...
    print(
        "ERROR: python-rich is not available in this Python environment.",
        file=sys.stderr,
    )
    ...
    print(
        "\nThis usually means python-rich is installed for a different Python version.",
        file=sys.stderr,
    )
    print(
        "The CLI requires system Python with distro packages installed.",
        file=sys.stderr,
    )
    ...
    print(
        "\nIf using a Python version manager (pyenv, mise, asdf), ensure",
        file=sys.stderr,
    )
    print(
        "python-rich is installed for your system Python (/usr/bin/python3).",
        file=sys.stderr,
    )
    sys.exit(1)
...
try:
    from .backend_installer import (
        ...
        get_all_state,
        init_state,
        _cleanup_partial_installation,
        PARAKEET_VENV_DIR,
        PARAKEET_SCRIPT,
        USER_BASE,
        PYWHISPERCPP_SRC_DIR,
...
RESUME_SERVICE_NAME = (
    "hyprwhspr-resume.service"  # Deprecated, kept for cleanup in uninstall/status
)
...
def _check_ydotool_version() -> tuple[bool, str, str]:
    ...
    try:
        result = subprocess.run(
            ["dpkg", "-l", "ydotool"], capture_output=True, text=True, timeout=5
        )
    ...
    if not version:
        try:
            result = subprocess.run(
                ["pacman", "-Q", "ydotool"], capture_output=True, text=True, timeout=5
            )
    ...
    if not version:
        try:
            result = subprocess.run(
                ["ydotool", "--version"], capture_output=True, text=True, timeout=5
            )
...
def _validate_hyprwhspr_root() -> bool:
    ...
    if not root_path.exists():
        ...
        if is_development:
            ...
            log_error(
                "Check that HYPRWHSPR_ROOT environment variable is set correctly."
            )
            log_error(
                "For development, ensure you're running from the repository root."
            )
            log_error("")
    ...
    if missing_files:
        log_error(
            f"HYPRWHSPR_ROOT is missing required files: {', '.join(missing_files)}"
        )
...
def _cleanup_backend(backend_type: str) -> bool:
    ...
    if backend_type == "parakeet":
        ...
        if parakeet_service_dest.exists():
            ...
            run_command(
                ["systemctl", "--user", "stop", PARAKEET_SERVICE_NAME], check=False
            )
            run_command(
                ["systemctl", "--user", "disable", PARAKEET_SERVICE_NAME], check=False
            )
...
def _prompt_backend_selection():
    ...
    print(
        "  [1] Parakeet TDT V3   - Solid performance for most people (Autodetects CPU/GPU)"
    )
    ...
    while True:
        try:
            choice = Prompt.ask(
                "Select backend", choices=["1", "2", "3", "4", "5", "6"], default="1"
            )
            backend_map = {
                "1": "onnx-asr",
                "2": "cpu",
            ...
            if current_backend and current_backend != selected:
                ...
                if current_backend not in [
                    "rest-api",
                    "remote",
                    "realtime-ws",
                ] and selected not in ["rest-api", "remote", "realtime-ws"]:
                    print(
                        "This will recreate the venv and install the new backend cleanly."
                    )
                    ...
                    backend_type_name = (
                        "REST API" if selected == "rest-api" else "Realtime WebSocket"
                    )
                    print(f"Switching to {backend_type_name} backend.")
                    print("The local backend venv will no longer be needed.")
                    cleanup_venv = Confirm.ask(
                        "Remove the venv to free up space?", default=False
                    )
                    print(
                    ...
                    return (
                        ...
                        cleanup_venv,
                        False,
                    )  # Return tuple: (backend, cleanup_venv, wants_reinstall)
...
def _prompt_model_selection():
    ...
    for i, (model, desc) in enumerate(
        english_only_models, len(multilingual_models) + 1
    ):
        print(f"  [{i}] {model:12} - {desc}")
    print()

    all_models = [m[0] for m in multilingual_models] + [
        m[0] for m in english_only_models
    ]
    ...
    while True:
        try:
            choice = Prompt.ask(
                "Select model", choices=choices, default="2"
            )  # default to base
        ...
        try:
            ...
            if choice_num <= len(providers_list):
                ...
                for model_id, model_data in models.items():
                    ...
                    print(
                        f"  [{len(model_list)}] {model_data['name']} - {model_data['description']}"
                    )
                ...
                if not model_list:
                    if filter_realtime:
                        ...
                        if not Confirm.ask(
                            "Select a different provider?", default=True
                        ):
                            return None
                        continue
                    else:
                ...
                model_choice = Prompt.ask(
                    "Select model", choices=model_choices, default="1"
                )
                selected_model_id, selected_model_data = model_list[
                    int(model_choice) - 1
                ]
                ...
                if existing_key:
                    ...
                    if use_existing:
                        ...
                        api_key = getpass.getpass(
                            f"Enter {provider['api_key_description']}: "
                        )
                else:
                    # Use getpass for secure password input (masks input, doesn't echo)
                    api_key = getpass.getpass(
                ...
                if save_credential(provider_id, api_key):
                    ...
                    log_warning(
                        "Failed to save API key, but continuing with configuration"
                    )
                ...
                print(
                    "Examples: whisper-1, whisper-large-v3, distil-whisper-large-v3-en"
                )
                ...
                if has_body:
                    body_json = Prompt.ask(
                        'Enter additional body fields as JSON (e.g., {"language": "en"})',
                        default="{}",
                    )
                    try:
...
def _generate_remote_config(
    ...
    if backend_type == "realtime-ws":
        ...
        if (
            provider_id == "custom"
            and custom_config
            and "websocket_url" in custom_config
        ):
            config["websocket_url"] = custom_config["websocket_url"]
        if provider_id == "custom" and custom_config:
            config["websocket_protocol"] = custom_config.get(
                "websocket_protocol", "vllm-realtime"
            )
            config["websocket_auth_mode"] = custom_config.get(
                "websocket_auth_mode", "none"
            )
            if custom_config.get("websocket_api_key_header"):
                config["websocket_api_key_header"] = custom_config.get(
                    "websocket_api_key_header"
                )
        return config
...
def setup_command(python_path: Optional[str] = None):
    ...
    if mise_active:
        ...
        if shutil.which("mise"):
            try:
                run_command(
                    ["bash", "-c", "mise deactivate"], check=False, capture_output=True
                )
            except Exception:
                pass
    ...
    if not python_path:  # Only check if user didn't specify a custom Python
        ...
        if guidance:
            ...
            if is_compatible:
                ...
                print()
                if not Confirm.ask(
                    "Continue anyway? (Cloud backends will still work)", default=True
                ):
                    log_info("Setup cancelled.")
                    log_info(
                        "Install Python 3.14 or 3.13, then re-run: hyprwhspr setup"
                    )
                    return
                print()
    ...
    if is_parakeet_config:
        ...
        if Confirm.ask(
            "\nMigrate to in-process onnx-asr backend (GPU-accelerated, no REST server)?",
            default=True,
        ):
            ...
            log_warning(
                "Keeping Parakeet REST backend. Note: Parakeet REST is deprecated."
            )
            log_warning(
                "Consider migrating to onnx-asr for better performance and simpler setup."
            )
    ...
    if backend_normalized not in ["rest-api", "remote", "realtime-ws"]:
        ...
        if current_backend == backend_normalized and not wants_reinstall:
            ...
            if backend_normalized == "onnx-asr":
                ...
                print(
                    "This backend automatically detects and uses GPU acceleration when available,"
                )
                print(
                    "or falls back to CPU-optimized mode. Uses ONNX runtime for fast transcription."
                )
                print(
                    "This may take several minutes as it downloads models and dependencies."
                )
            else:
                print(
            ...
            if not Confirm.ask("Proceed with backend installation?", default=True):
                ...
                log_warning(
                    "Backend installation is required for local transcription to work."
                )
            else:
                ...
                if not install_backend(
                    backend_normalized,
                    force_rebuild=wants_reinstall,
                    custom_python=python_path,
                ):
    ...
    if backend_normalized in ["rest-api", "remote"]:
        ...
        try:
            remote_config = _generate_remote_config(
                provider_id, model_id, api_key, custom_config, backend_type="rest-api"
            )
        ...
        if provider_id != "custom":
            ...
            if models:
                ...
                is_realtime_model = (
                    model_data.get("realtime_model", False)
                    or "realtime" in model_id.lower()
                )

                if not is_realtime_model:
                    log_warning(
                        f"Selected model {model_id} may not be realtime-compatible"
                    )
                    # Try to find a realtime model
                    realtime_models = [
                        mid
                        for mid in models.keys()
                        if models[mid].get("realtime_model", False)
                        or "realtime" in mid.lower()
                    ]
                    if realtime_models:
                        if not Confirm.ask(
                            f"Use realtime model '{realtime_models[0]}' instead?",
                            default=True,
                        ):
                            log_error(
                                "Realtime-compatible model required for realtime-ws backend"
                            )
                            return
                        ...
                        log_error(
                            f"No realtime-compatible models found for provider {provider_id}"
                        )
                        return
        ...
        if provider_id == "custom":
            ...
            websocket_url = Prompt.ask(
                "WebSocket URL (e.g., wss://api.example.com/v1/realtime)", default=""
            )
            ...
            print("\nRealtime protocol:")
            print("  1. vLLM-compatible (recommended for local Voxtral/vLLM)")
            print("  2. OpenAI-compatible")
            protocol_choice = Prompt.ask(
                "Select protocol", choices=["1", "2"], default="1"
            )
            custom_config["websocket_protocol"] = (
                "vllm-realtime" if protocol_choice == "1" else "openai-realtime"
            )

            print("\nAuthentication mode:")
            print("  1. none (local server, no auth)")
            print("  2. bearer (Authorization: Bearer <key>)")
            print("  3. header (custom header key)")
            auth_choice = Prompt.ask(
                "Select auth mode", choices=["1", "2", "3"], default="1"
            )
            auth_mode = {"1": "none", "2": "bearer", "3": "header"}[auth_choice]
            custom_config["websocket_auth_mode"] = auth_mode

            if auth_mode == "header":
                header_name = Prompt.ask("API key header name", default="X-API-Key")
                custom_config["websocket_api_key_header"] = header_name
            ...
            if not model_id:
                ...
                if not model_id:
                    log_error(
                        "Model identifier is required for custom realtime backends"
                    )
                    return
    ...
    if backend_normalized in ["rest-api", "remote", "realtime-ws"]:
        ...
        print("=" * 60)
        log_info(
            "Ensuring Python virtual environment and dependencies are installed..."
        )
        ...
        if cur_req_hash != stored_req_hash or not stored_req_hash or not deps_installed:
            if not stored_req_hash:
                ...
                log_info(
                    "Installing base Python dependencies (excluding pywhispercpp)..."
                )
            elif cur_req_hash != stored_req_hash:
                # Requirements actually changed
                log_info(
                    "Requirements.txt has changed. Updating base Python dependencies..."
                )
            else:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                try:
                    ...
                    if temp_req_path.stat().st_size > 0:
                        run_command(
                            [str(pip_bin), "install", "-r", str(temp_req_path)],
                            check=True,
                        )
                    else:
    ...
    if waybar_installed:
        ...
        setup_waybar_choice = Confirm.ask(
            "Set up Waybar integration anyway?", default=False
        )
    ...
    if mic_osd_available:
        setup_mic_osd_choice = Confirm.ask(
            "Enable mic-osd visualization?", default=True
        )
    else:
        ...
        if Path("/etc/debian_version").exists():
            ...
            and "suse" in Path("/etc/os-release").read_text().lower()
        ):
        ...
        setup_mic_osd_choice = Confirm.ask(
            "Enable mic-osd anyway (will work after installing deps)?", default=False
        )
    ...
    hypr_config_exists = (hypr_config_dir / "hyprland.conf").exists() or (
        hypr_config_dir / "bindings.conf"
    ).exists()
    ...
    if is_hyprland_session or hypr_config_exists or "hyprland" in current_desktop:
        ...
        print(
            "Requires adding bindings to ~/.config/hypr/hyprland.conf or bindings.conf"
        )

        if is_hyprland_session:
            print("\nHyprland session detected.")
            setup_hyprland_choice = Confirm.ask(
                "Configure Hyprland compositor bindings?", default=True
            )
            ...
            setup_hyprland_choice = Confirm.ask(
                "Set up Hyprland compositor bindings anyway?", default=False
            )
    else:
    ...
    if remote_config:
        ...
        if provider_id:
            ...
            if api_key:
                ...
                print(
                    f"Provider: {provider_id} (API Key: not found in credential store)"
                )
    ...
    try:
        ...
        if _is_running_manually():
            ...
            log_warning(
                "Please restart it manually for configuration changes to take effect."
            )
        ...
        if setup_systemd_choice:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    log_info(
                        "Restarting hyprwhspr service to apply configuration changes..."
                    )
                    systemd_restart()
            except (
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    log_warning(
                        "You may need to manually restart the service for changes to take effect:"
                    )
        ...
        log_info(
            "Partial setup completed. You can resume by running 'hyprwhspr setup' again."
        )
        sys.exit(1)
    except Exception as e:
        ...
        log_info(
            "You can try running 'hyprwhspr backend repair' to fix issues, or 'hyprwhspr state reset' to start fresh."
        )
        sys.exit(1)
...
def _auto_download_model(model: str = "base"):
    ...
    if download_pywhispercpp_model(model):
        ...
        log_warning(
            f"Model download failed - can download later with: hyprwhspr model download {model}"
        )
...
def _setup_hyprland_bindings() -> bool:
    ...
    if bindings_file.exists():
        ...
        try:
            ...
            log_warning(
                "Skipping Hyprland bindings setup - see README for manual setup"
            )
            return False
    ...
    if target_file:
        ...
        try:
            if target_file.exists():
                with open(target_file, "r", encoding="utf-8") as f:
                    ...
                    if (
                        "hyprwhspr-tray.sh record" in content
                        or "# added by hyprwhspr" in content
                    ):
                        bindings_exist = True
        except Exception as e:
        ...
        if bindings_exist:
            ...
            try:
                ...
                log_warning(
                    "Could not add bindings automatically - see README for manual setup"
                )
                return False
            except Exception as e:
...
def omarchy_command(args=None):
    ...
    print("=" * 60)
    if not _verify_installation_step(
        "Backend installation", lambda: _verify_backend_installation(backend)
    ):
        log_error(
            "Backend installation verification failed - installation may be incomplete"
        )
        return False
    ...
    if backend in ["cpu", "nvidia", "vulkan"]:
        ...
        if not _verify_installation_step(
            "Model download", lambda: _verify_model_downloaded(model_to_download)
        ):
            log_warning(
                "Model download verification failed - model may not be available"
            )
            log_warning(
                f"You can download it later with: hyprwhspr model download {model_to_download}"
            )
...
def configure_secondary_shortcut():
    ...
    if current_shortcut:
        ...
        print()
        if not Confirm.ask(
            "Do you want to change the secondary shortcut?", default=False
        ):
            return
    ...
    print(
        "Examples: 'it' (Italian), 'en' (English), 'fr' (French), 'de' (German), 'es' (Spanish)"
    )
    print("Leave blank to disable secondary shortcut.")
    ...
    log_success(
        f"Secondary shortcut configured: {shortcut.strip()} (language: {language})"
    )
...
def setup_systemd(mode: str = "install"):
    ...
    if mode in ("install", "enable"):
        ...
        run_command(
            ["systemctl", "--user", "enable", "--now", YDOTOOL_UNIT], check=False
        )
        ...
        if service_was_running:
            log_info(
                "Service was already running, restarting to apply configuration changes..."
            )
            systemd_restart()
        else:
...
def systemd_status():
    ...
    try:
        ...
        if (USER_SYSTEMD_DIR / RESUME_SERVICE_NAME).exists():
            ...
            run_command(
                ["systemctl", "--user", "status", RESUME_SERVICE_NAME], check=False
            )
...
def setup_waybar(mode: str = "install"):
    ...
    if mode == "install":
        ...
        try:
            ...
            log_error(
                f"Failed to parse waybar config.jsonc (after stripping comments): {e}"
            )
        ...
        if waybar_style.exists():
            ...
            log_warning(
                "No waybar style.css found - user will need to add CSS import manually"
            )
        ...
        if waybar_config.exists():
            try:
                ...
                if (
                    "modules-right" in config
                    and "custom/hyprwhspr" in config["modules-right"]
                ):
...
def model_command(
    ...
    if action == "download":
        if repo_id:
            return model_download_command(repo_id=repo_id, filename=filename)
        download_model(model_name)
    elif action == "list":
        list_models()
    elif action == "list-local":
...
def _get_whisper_model_catalog() -> list[str]:
    """Get all known pywhispercpp model names supported by this CLI."""
    return [
        "tiny",
        "base",
        "small",
...
def _is_whisper_model_installed(model_name: str) -> bool:
    """Check whether a pywhispercpp model file exists and looks valid."""
    model_file = PYWHISPERCPP_MODELS_DIR / f"ggml-{model_name}.bin"
    if not model_file.exists() and not model_name.endswith(".en"):
        model_file = PYWHISPERCPP_MODELS_DIR / f"ggml-{model_name}.en.bin"
    return model_file.exists() and model_file.stat().st_size > 10_000_000


def set_active_model(model_name: str) -> bool:
    """Set active model based on the currently configured backend."""
    config = ConfigManager()
    backend = normalize_backend(
        config.get_setting("transcription_backend", "pywhispercpp")
    )

    if backend in ("pywhispercpp", "cpu", "nvidia", "vulkan", "amd"):
        if not _is_whisper_model_installed(model_name):
            catalog = _get_whisper_model_catalog()
            if model_name not in catalog:
                log_error(f"Unknown Whisper model: {model_name}")
                log_info(
                    "Run 'hyprwhspr model list' to see supported downloadable models"
                )
                return False

            log_warning(f"Model '{model_name}' is not installed")
            if not Confirm.ask(f"Download '{model_name}' now?", default=True):
                log_warning("Model switch cancelled")
                return False
        ...
        config.set_setting("model", model_name)
        config.save_config()
        log_success(f"Active Whisper model set to: {model_name}")
        return True

    if backend == "onnx-asr":
        config.set_setting("onnx_asr_model", model_name)
        config.save_config()
        log_success(f"Active ONNX-ASR model set to: {model_name}")
        log_info("Model will be downloaded by onnx-asr on first use if needed")
        return True

    if backend == "rest-api":
        rest_body = config.get_setting("rest_body", {})
        if not isinstance(rest_body, dict):
            rest_body = {}
        rest_body["model"] = model_name
        config.set_setting("rest_body", rest_body)
        config.save_config()
        log_success(f"REST model set to: {model_name}")
        return True

    if backend == "realtime-ws":
        config.set_setting("websocket_model", model_name)

        active_profile = config.get_setting("active_local_profile")
        profiles = config.get_setting("local_model_profiles", {})
        if active_profile and isinstance(profiles, dict) and active_profile in profiles:
            ...
            if isinstance(profile, dict):
                ...
                config.set_setting("local_model_profiles", profiles)

        config.save_config()
        log_success(f"Realtime model set to: {model_name}")
        return True

    log_error(f"Model switching is not supported for backend: {backend}")
    return False


def show_current_model() -> None:
    """Show currently active model for the configured backend."""
    config = ConfigManager()
    backend = normalize_backend(
        config.get_setting("transcription_backend", "pywhispercpp")
    )
    ...
    if backend in ("pywhispercpp", "cpu", "nvidia", "vulkan", "amd"):
        ...
        installed = _is_whisper_model_installed(model_name)
    ...
    if backend == "rest-api":
        rest_body = config.get_setting("rest_body", {})
        model_name = rest_body.get("model") if isinstance(rest_body, dict) else None
        print(f"Backend: {backend}")
        print(f"Model: {model_name or '(not set)'}")
        return

    if backend == "realtime-ws":
        model_name = config.get_setting("websocket_model")
        profile_name = config.get_setting("active_local_profile")
        print(f"Backend: {backend}")
        print(f"Model: {model_name or '(not set)'}")
        if profile_name:
...
def list_models():
    ...
    print(
        "      Use English-only (.en) models for English-only content (smaller file size)."
    )
...
def model_download_command(repo_id: str, filename: Optional[str] = None) -> bool:
    """Download local model assets from HuggingFace."""
    try:
        try:
        ...
        if not filename:
            log_info(f"Downloading transformers repo snapshot: {repo_id}")
            model_dir = manager.download_transformers_repo(repo_id=repo_id)
            log_success(f"Downloaded repo to: {model_dir}")
            return True

        log_info(f"Downloading GGUF model: {repo_id}/{filename}")

        def _progress(
            downloaded_size_bytes: int, total_size_bytes: Optional[int]
        ) -> None:
            if not OutputController.is_progress_enabled():
                return
            if total_size_bytes and total_size_bytes > 0:
                percent = min(
                    100, int((downloaded_size_bytes * 100) / total_size_bytes)
                )
                downloaded_mb = downloaded_size_bytes / (1024 * 1024)
                total_mb = total_size_bytes / (1024 * 1024)
                OutputController.write(
                    f"\r[INFO] Downloading GGUF: {downloaded_mb:.1f}/{total_mb:.1f} MB ({percent}%)",
                ...
                downloaded_mb = downloaded_size_bytes / (1024 * 1024)
                OutputController.write(
                    f"\r[INFO] Downloading GGUF: {downloaded_mb:.1f} MB",
                    VerbosityLevel.NORMAL,
                    flush=True,
                )
        ...
        OutputController.write("\n", VerbosityLevel.NORMAL, flush=True)
        log_success(f"Downloaded model: {model_path}")
        return True
    except Exception as error_value:
        OutputController.write("\n", VerbosityLevel.NORMAL, flush=True)
        log_error(f"Model download failed: {error_value}")
        return False


def model_list_local_command() -> None:
    """List cached local GGUF models."""
    try:
        try:
            from .local_model_manager import LocalModelManager
        except ImportError:
        ...
        print("Cached local GGUF models:")
        for item in models:
            size_gb = item.size_bytes / (1024 * 1024 * 1024)
            print(f"  - {item.name}")
            print(f"      path={item.path}")
            print(f"      size={size_gb:.2f} GB")
            print(f"      context={item.context_size}")
    except Exception as error_value:
        log_error(f"Failed to list cached models: {error_value}")


def model_recommend_command() -> None:
    """Show GPU availability and recommended quantization."""
    try:
        try:
            from .local_model_manager import LocalModelManager
        except ImportError:
        ...
        manager = LocalModelManager()
        has_gpu, gpu_type, vram_mb = manager.detect_gpu()
        if not has_gpu:
            log_warning("No supported GPU detected")
            print("Recommended quantization: Q4_K_M (conservative)")
            return

        quantization = manager.recommend_quantization(vram_mb)
        print(f"GPU type: {gpu_type}")
        print(f"VRAM: {vram_mb} MB")
        print(f"Recommended quantization: {quantization}")
    except Exception as error_value:
        log_error(f"Failed to detect GPU: {error_value}")
...
def server_start_command(
    ...
    """Start local model websocket server."""
    try:
        try:
            from .model_server_controller import ModelServerController
        except ImportError:
            from model_server_controller import ModelServerController

        config = ConfigManager()
        server_config = config.get_setting("local_model_server", {})
        if not isinstance(server_config, dict):
            server_config = {}

        resolved_model_path = model_path or server_config.get("model_path")
        if not resolved_model_path:
            log_error("No local model path configured")
            log_info(
                "Run: hyprwhspr setup --local-model --repo <repo> (or --local-gguf --local-path <file.gguf>)"
            )
            return False

        controller = ModelServerController()
        model_path_obj = Path(resolved_model_path).expanduser().resolve()
        host = str(server_config.get("host", "127.0.0.1"))
        source_type = str(server_config.get("source_type", "gguf"))
        n_gpu_layers = int(server_config.get("n_gpu_layers", -1))
        n_ctx = int(server_config.get("n_ctx", 8192))
        port_value = int(port or server_config.get("port", 8000))
        idle_timeout_value = int(idle_timeout or server_config.get("idle_timeout", 300))
        ...
        if controller.start_server(
            ...
            log_success(
                f"Local model server ready at ws://{host}:{port_value}/v1/realtime"
            )
            return True

        log_error("Failed to start local model server")
        return False
    except Exception as error_value:
        log_error(f"Failed to start server: {error_value}")
        return False


def server_stop_command() -> bool:
    """Stop local model websocket server."""
    try:
        try:
            from .model_server_controller import ModelServerController
        except ImportError:
        ...
        controller = ModelServerController()
        if controller.stop_server():
            log_success("Local model server stopped")
            return True
        log_error("Failed to stop local model server")
        return False
    except Exception as error_value:
        log_error(f"Failed to stop server: {error_value}")
        return False


def server_status_command() -> None:
    """Show local model server status and log location."""
    try:
        try:
            from .model_server_controller import ModelServerController
        except ImportError:
        ...
        print(f"PID: {status.pid if status.pid else '(none)'}")
        ...
        if MODEL_SERVER_LOG_FILE.exists():
            try:
                log_lines = MODEL_SERVER_LOG_FILE.read_text(
                    encoding="utf-8"
                ).splitlines()
                tail_lines = log_lines[-10:]
                if tail_lines:
                    print("Recent logs:")
        ...
        log_error(f"Failed to get server status: {error_value}")


def setup_local_gguf_command(
    source: str,
    filename: Optional[str] = None,
    gpu_layers: int = -1,
    port: int = 8000,
) -> bool:
    """Configure local GGUF backend from HF repo or local path."""
    try:
        try:
            from .local_model_manager import LocalModelManager
        except ImportError:
        ...
        manager = LocalModelManager()
        if source.startswith("/") or source.startswith(".") or source.endswith(".gguf"):
            valid, result_message = manager.validate_local_model(source)
            if not valid:
                log_error(result_message)
                return False
            model_path_obj = Path(result_message)
        else:
            if not filename:
                log_error("filename is required when source is a HuggingFace repo")
                return False
            if not model_download_command(source, filename):
                return False
            model_path_obj = manager.cache_dir / filename

        profile_name = _configure_local_model_profile(
            model_path_obj=model_path_obj,
            gpu_layers=gpu_layers,
            port=port,
            source_type="gguf",
            source_path=str(model_path_obj),
        )

        log_success("Local GGUF configuration saved")
        log_info(f"Profile: {profile_name}")
        log_info(f"Model path: {model_path_obj}")
        return True
    except Exception as error_value:
        log_error(f"Failed to configure local GGUF: {error_value}")
        return False


def setup_local_model_command(
    source: str,
    filename: Optional[str] = None,
    gpu_layers: int = -1,
    port: int = 8000,
) -> bool:
    """Configure local model from safetensors (primary) or GGUF sources."""
    try:
        try:
            from .local_model_manager import LocalModelManager
        except ImportError:
        ...
        if source.startswith("/") or source.startswith("."):
            ...
            if path_obj.is_file() and path_obj.suffix.lower() == ".gguf":
                return setup_local_gguf_command(
                    source=str(path_obj),
                    filename=None,
                    gpu_layers=gpu_layers,
                    port=port,
            ...
            valid, message = manager.validate_local_transformers_model(str(path_obj))
            if not valid:
                log_error(message)
                return False
            transformers_dir = Path(message)
        else:
            if filename:
                return setup_local_gguf_command(
                    source=source,
            ...
            log_info(
                "Downloading safetensors model snapshot from HuggingFace (this can take several minutes)"
            )
            transformers_dir = manager.download_transformers_repo(source)
            log_success(f"Downloaded model snapshot: {transformers_dir}")
            source_path = source

        model_path_obj = transformers_dir
        ...
        log_success("Local model configuration saved (native safetensors runtime)")
        log_info(f"Profile: {profile_name}")
        log_info(f"Source: {source_path}")
        log_info(f"Runtime model dir: {model_path_obj}")
        return True
    except Exception as error_value:
        log_error(f"Failed to configure local model: {error_value}")
        return False


def _configure_local_model_profile(
    model_path_obj: Path,
    gpu_layers: int,
    port: int,
    source_type: str,
    ...
    n_ctx = 8192
    if source_type == "gguf":
        n_ctx = manager.get_model_context_size(model_path_obj)
    config = ConfigManager()
    local_server_config = config.get_setting("local_model_server", {})
    if not isinstance(local_server_config, dict):
        local_server_config = {}
    local_server_config.update(
        {
            ...
            "port": port,
            "host": "127.0.0.1",
            "idle_timeout": int(local_server_config.get("idle_timeout", 300)),
            "auto_start": True,
            "max_restarts": int(local_server_config.get("max_restarts", 3)),
        }
    )
    config.set_setting("local_model_server", local_server_config)

    profile_name = f"{model_path_obj.name}-local"
    profiles = config.get_setting("local_model_profiles", {})
    if not isinstance(profiles, dict):
        profiles = {}
    profiles[profile_name] = {
        "websocket_url": f"ws://127.0.0.1:{port}/v1/realtime",
        "model": model_path_obj.name,
    ...
    config.set_setting("local_model_profiles", profiles)
    config.set_setting("active_local_profile", profile_name)
    config.set_setting("transcription_backend", "realtime-ws")
    config.set_setting("websocket_provider", "custom")
    config.set_setting("websocket_model", model_path_obj.name)
    config.set_setting("websocket_url", f"ws://127.0.0.1:{port}/v1/realtime")
    config.set_setting("websocket_protocol", "vllm-realtime")
    config.set_setting("websocket_auth_mode", "none")
    config.set_setting("websocket_api_key_header", None)
    config.save_config()
    return profile_name
...
def status_command():
    ...
    if config_file.exists():
        ...
        try:
            ...
            log_warning(
                f"Config file is invalid, hyprwhspr will be using default config. Please check config line {e.lineno}, column  {e.colno}."
            )
    else:
...
def setup_permissions():
    ...
    try:
        result = run_sudo_command(
            ["usermod", "-a", "-G", "input,audio,tty", username], check=False
        )
        if result.returncode == 0:
            ...
            log_info(
                "You may need to run manually: sudo usermod -a -G input,audio,tty $USER"
            )
            any_failures = True
    except Exception as e:
    ...
    if not udev_rule.exists():
        ...
        try:
            result = run_sudo_command(
                ["tee", str(udev_rule)], input_data=rule_content.encode(), check=False
            )
            if result.returncode == 0:
                log_success("udev rule created")
            else:
                log_warning(
                    f"Failed to create udev rule (exit code {result.returncode})"
                )
                log_info(
                    "You may need to run manually: sudo tee /etc/udev/rules.d/99-uinput.rules"
                )
                any_failures = True
        except Exception as e:
    ...
    # Reload udev
    try:
        result1 = run_sudo_command(
            ["udevadm", "control", "--reload-rules"], check=False
        )
        result2 = run_sudo_command(
            ["udevadm", "trigger", "--name-match=uinput"], check=False
        )
        if result1.returncode == 0 and result2.returncode == 0:
            ...
            log_info(
                "You may need to run manually: sudo udevadm control --reload-rules && sudo udevadm trigger"
            )
            any_failures = True
    except Exception as e:
    ...
    if any_failures:
        log_warning(
            "Some permission setup commands failed. You may need to run them manually as root."
        )
...
def backend_repair_command():
    ...
    try:
        config_manager = ConfigManager()
        configured_backend = config_manager.get_setting(
    ...
    if backend_module and venv_python.exists() and not venv_corrupted:
        try:
            ...
            if result.returncode != 0:
                log_warning(
                    f"{backend_module} is not installed in venv (required for {configured_backend} backend)"
                )
    ...
    if backend_missing:
        print("\nIssues found:")
        print(
            f"  • {backend_module} is not installed (required for {configured_backend} backend)"
        )
        print("\nOptions:")
        ...
        if choice == "1":
            ...
            if configured_backend and configured_backend in [
                "cpu",
                "nvidia",
                "amd",
                "vulkan",
                ...
                log_warning(
                    "Could not detect backend type. Please run 'hyprwhspr setup'"
                )
                return False
...
def backend_list_profiles_command() -> bool:
    """List configured local model profiles."""
    config = ConfigManager()
    profiles = config.get_setting("local_model_profiles", {})
    active = config.get_setting("active_local_profile")

    if not isinstance(profiles, dict) or not profiles:
        log_info("No local model profiles configured")
        log_info(
            "Add one with: hyprwhspr backend add-profile <name> --url ... --model ..."
        )
        return True

    print("Local model profiles:")
    for name in sorted(profiles.keys()):
        entry = profiles.get(name, {})
        if not isinstance(entry, dict):
            continue
        url = entry.get("websocket_url", "(missing-url)")
        model = entry.get("model", "(missing-model)")
        protocol = entry.get("protocol", "openai-realtime")
        auth_mode = entry.get("auth_mode", "none")
        auto_start_server = bool(entry.get("auto_start_server", False))
        marker = " (active)" if name == active else ""
        print(f"  - {name}{marker}")
        print(f"      model={model}")
        print(f"      protocol={protocol}")
        print(f"      auth_mode={auth_mode}")
        print(f"      auto_start_server={auto_start_server}")
        print(f"      websocket_url={url}")

    return True


def backend_add_profile_command(
    name: str,
    websocket_url: str,
    model: str,
    protocol: str = "vllm-realtime",
    auth_mode: str = "none",
    api_key_header: Optional[str] = None,
    auto_start_server: bool = True,
    model_path: Optional[str] = None,
) -> bool:
    """Add or update a local realtime model profile."""
    config = ConfigManager()
    profiles = config.get_setting("local_model_profiles", {})
    if not isinstance(profiles, dict):
        profiles = {}
    ...
    if not websocket_url.startswith("ws://") and not websocket_url.startswith("wss://"):
        log_error("WebSocket URL must start with ws:// or wss://")
        return False

    if auth_mode == "header" and not api_key_header:
    ...
    profiles[name] = {
        ...
        "auto_start_server": bool(auto_start_server),
        "model_path": model_path,
    }

    config.set_setting("local_model_profiles", profiles)
    config.save_config()
    log_success(f"Saved profile: {name}")
    log_info(f"Use it with: hyprwhspr backend use-profile {name}")
    return True


def backend_use_profile_command(name: str) -> bool:
    """Apply a local model profile to current backend configuration."""
    config = ConfigManager()
    profiles = config.get_setting("local_model_profiles", {})
    if not isinstance(profiles, dict) or name not in profiles:
        log_error(f"Profile not found: {name}")
        log_info("Run 'hyprwhspr backend list-profiles' to see available profiles")
        return False

    profile = profiles.get(name)
    if not isinstance(profile, dict):
        log_error(f"Invalid profile data for: {name}")
        return False

    websocket_url = profile.get("websocket_url")
    model = profile.get("model")
    if not websocket_url or not model:
        log_error(f"Profile '{name}' is missing required fields (websocket_url/model)")
        return False

    provider_id = "custom"
    protocol = profile.get("protocol", "vllm-realtime")
    auth_mode = profile.get("auth_mode", "none")
    api_key_header = profile.get("api_key_header")

    config.set_setting("transcription_backend", "realtime-ws")
    config.set_setting("websocket_provider", provider_id)
    config.set_setting("websocket_model", model)
    config.set_setting("websocket_url", websocket_url)
    config.set_setting("websocket_protocol", protocol)
    config.set_setting("websocket_auth_mode", auth_mode)
    config.set_setting("websocket_api_key_header", api_key_header)
    config.set_setting("active_local_profile", name)
    config.save_config()

    if _is_loopback_websocket_url(websocket_url) and bool(
        profile.get("auto_start_server", True)
    ):
        try:
            server_config = config.get_setting("local_model_server", {})
            if not isinstance(server_config, dict):
                server_config = {}
            profile_model_path = profile.get("model_path")
            model_path = profile_model_path or server_config.get("model_path")
            if model_path:
                try:
                    from .model_server_controller import ModelServerController
                except ImportError:
                ...
                if not controller.is_running():
                    ...
                    idle_timeout = int(server_config.get("idle_timeout", 300))
                    n_gpu_layers = int(server_config.get("n_gpu_layers", -1))
                    n_ctx = int(server_config.get("n_ctx", 8192))
                    if controller.start_server(
                        ...
                        log_info("Auto-started local model server")
                    else:
                        log_warning("Could not auto-start local model server")
            else:
                log_warning(
                    "Profile uses localhost but no model_path is configured in profile or local_model_server"
                )
        except Exception as error_value:
            log_warning(f"Auto-start check failed: {error_value}")

    log_success(f"Applied profile: {name}")
    log_info(f"Realtime URL: {websocket_url}")
    log_info(f"Model: {model}")
    log_info("Restart service for a clean reconnect: hyprwhspr systemd restart")
    return True


def _is_loopback_websocket_url(websocket_url: str) -> bool:
    return (
        websocket_url.startswith("ws://127.0.0.1")
        or websocket_url.startswith("ws://localhost")
        or websocket_url.startswith("wss://127.0.0.1")
        or websocket_url.startswith("wss://localhost")
    )
    ...
    if state == "completed":
        if not venv_python.exists():
            ...
            try:
                ...
                if result.returncode != 0:
                    issues.append(
                        "State says 'completed' but pywhispercpp is not installed"
                    )
            except Exception:
                pass
...
def validate_command():
    ...
    is_pywhispercpp = current_backend in [
        "cpu",
        "nvidia",
        "amd",
        "vulkan",
    ...
    if is_onnx_asr:
        ...
        if pywhispercpp_available:
            ...
            print(
                "(or use REST API backend by setting 'transcription_backend': 'rest-api' in config.json)"
            )
            print("")
    ...
    try:
        ...
        if use_hypr_bindings:
            ...
            if grab_keys:
                ...
                log_warning(
                    "  Recommendation: Set grab_keys=false when using compositor bindings"
                )
    except Exception:
...
def test_command(live: bool = False, mic_only: bool = False):
    ...
    if backend == "rest-api":
        ...
        auth_mode = config.get_setting("websocket_auth_mode", "bearer")
        ...
        if not provider_id:
            ...
            if auth_mode == "none":
                ...
                log_success("No credentials required (auth mode: none)")
                backend_ready = True
            else:
                api_key = get_credential(provider_id)
                if api_key:
            ...
            if backend_ready and _is_loopback_websocket_url(
                config.get_setting("websocket_url", "") or ""
            ):
                log_success(f"Provider: {provider_id}, Model: {model_id}")
                log_info("Loopback realtime server configured")
            ...
            if audio and audio.is_available():
                ...
                try:
                    ...
                    if audio_data is not None and len(audio_data) > 0:
                        ...
                        log_success(
                            f"Recorded {len(audio_data) / 16000:.1f}s audio (level: {db:.0f}dB)"
                        )
        ...
        if audio_data is not None and len(audio_data) > 0:
            ...
            try:
                ...
                if not whisper.initialize():
                    ...
                    if duration > 5:
                        log_info(
                            f"Transcribing {duration:.0f}s of audio (this may take a moment)..."
                        )
                    else:
    ...
    try:
        ...
        if selected_device_name:
            ...
            for i, kb in enumerate(keyboards):
                ...
                if (
                    kb_name_lower == search_name_lower
                    or search_name_lower in kb_name_lower
                ):
...
def test_keyboard_access():
    ...
    try:
        ...
        if not results["accessible_devices"]:
            ...
            print(
                "  3. Consider using 'selected_device_name' in config to avoid conflicts"
            )
...
def uninstall_command(
    ...
    if not keep_models and PYWHISPERCPP_MODELS_DIR.exists():
        ...
        if models:
            items_to_remove.append(
                f"Whisper models: {len(models)} model(s) in {PYWHISPERCPP_MODELS_DIR}"
            )
    ...
    if not yes:
        log_warning(
            "This will permanently delete all hyprwhspr data and configuration."
        )
    ...
    try:
        ...
        if (USER_SYSTEMD_DIR / RESUME_SERVICE_NAME).exists():
            run_command(
                ["systemctl", "--user", "stop", RESUME_SERVICE_NAME], check=False
            )
            run_command(
                ["systemctl", "--user", "disable", RESUME_SERVICE_NAME], check=False
            )
        ...
        try:
            if PYWHISPERCPP_MODELS_DIR.exists():
                ...
                if models:
                    ...
                    log_success(
                        f"Removed {len(models)} model(s) from {PYWHISPERCPP_MODELS_DIR}"
                    )
                else:
            ...
            try:
                ...
                if not username:
                    ...
                    for group in groups_to_remove:
                        try:
                            run_sudo_command(
                                ["gpasswd", "-d", username, group], check=False
                            )
                            log_success(f"Removed user from '{group}' group")
                        except Exception as e:
                            log_warning(
                                f"Failed to remove user from '{group}' group: {e}"
                            )
                    ...
                    if udev_rule.exists():
                        ...
                        try:
                            with open(udev_rule, "r", encoding="utf-8") as f:
                                content = f.read()
                            if (
                                "hyprwhspr" in content.lower()
                                or "input" in content.lower()
                            ):
    ...
    if not skip_permissions and not permissions_removed:
        print(
            "Note: System permissions (group memberships, udev rules) were not removed."
        )
...
def record_command(action: str, language: str = None):
    ...
    if action == "start":
        ...
        if send_control(start_cmd):
            msg = (
                f"Recording started (language: {language})"
                if language
