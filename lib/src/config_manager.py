...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self.default_config = {
            ...
            "websocket_protocol": "openai-realtime",  # 'openai-realtime' or 'vllm-realtime'
            "websocket_auth_mode": "bearer",  # 'bearer' | 'header' | 'none'
            "websocket_api_key_header": None,  # Custom header name when websocket_auth_mode='header'
            ...
            # Local model profile management (primarily for local realtime servers)
            "local_model_profiles": {},  # Named profiles: {"voxtral-local": {...}}
            "active_local_profile": None,  # Last profile applied via CLI
            "local_model_server": {
                "model_path": None,
    ...
    def _load_config(self):
        ...
        try:
            if self.config_file.exists():
                ...
                default_local_model_server = self.default_config.get(
                    "local_model_server", {}
                )
                loaded_local_model_server = self.config.get("local_model_server", {})
                if isinstance(default_local_model_server, dict):
                    if not isinstance(loaded_local_model_server, dict):
                        loaded_local_model_server = {}
                    merged_local_model_server = default_local_model_server.copy()
                    merged_local_model_server.update(loaded_local_model_server)
                    self.config["local_model_server"] = merged_local_model_server
