...
class WhisperManager:
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        self._local_server_restart_attempts = 0
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if backend == "onnx-asr":
                ...
                os.environ["ORT_LOGGING_LEVEL"] = (
                    "4"  # 4 = FATAL (suppress ERROR/WARNING/INFO)
                )
                ...
                print(
                    f"[BACKEND] Loading onnx-asr model: {model_name} ({'GPU' if use_gpu else 'CPU'})",
                    flush=True,
                )
                ...
                try:
                    ...
                    if use_vad:
                        print(
                            "[BACKEND] Loading Silero VAD for long audio support",
                            flush=True,
                        )
                    ...
                    print(
                        f"[BACKEND] onnx-asr ready (model={model_name}, quantization={quantization}, vad={use_vad}, gpu={use_gpu})",
                        flush=True,
                    )
            ...
            if backend == "realtime-ws":
                ...
                websocket_auth_mode = self.config.get_setting(
                    "websocket_auth_mode", "bearer"
                )
                websocket_api_key_header = self.config.get_setting(
                    "websocket_api_key_header", None
                )
                websocket_protocol = self.config.get_setting(
                    "websocket_protocol", "openai-realtime"
                )

                if not provider_id:
                    print(
                        "ERROR: Realtime WebSocket backend selected but websocket_provider not configured"
                    )
                    return False

                if not model_id:
                    print(
                        "ERROR: Realtime WebSocket backend selected but websocket_model not configured"
                    )
                    return False

                # Get API key from credential manager (optional for custom local endpoints)
                api_key = None
                if websocket_auth_mode != "none":
                    api_key = get_credential(provider_id)
                    if not api_key:
                        print(
                            f"ERROR: Provider {provider_id} configured but API key not found in credential store"
                        )
                        return False
                ...
                if provider_id == "elevenlabs":
                    ...
                    buffer_max = self.config.get_setting(
                    ...
                    if not self._realtime_client.connect(
                        websocket_url, api_key, model_id, None
                    ):
                        print(
                            "ERROR: Failed to connect to ElevenLabs Realtime WebSocket"
                        )
                        try:
                    ...
                    realtime_mode = self.config.get_setting(
                        "realtime_mode", "transcribe"
                    )
                    self._realtime_client = RealtimeClient(
                        mode=realtime_mode, protocol=websocket_protocol
                    )
                    ...
                    if not websocket_url:
                        ...
                        if provider_id == "custom":
                            print(
                                "ERROR: Custom realtime backend requires websocket_url to be configured"
                            )
                            return False
                        ...
                        try:
                            websocket_url = self._get_websocket_url(
                                provider_id, model_id, realtime_mode
                            )
                        except Exception as e:
                    ...
                    # Connect
                    if provider_id == "custom" and self._is_loopback_websocket_url(
                        websocket_url
                    ):
                        if not self._ensure_local_ws_server_running(websocket_url):
                            return False
                    ...
                    self._realtime_connect_params = {
                        ...
                        "api_key_header": websocket_api_key_header,
                    }
                    if not self._realtime_client.connect(
                        websocket_url,
                        api_key,
            ...
            if backend == "rest-api":
                ...
                if not endpoint_url:
                    print(
                        "ERROR: REST backend selected but rest_endpoint_url not configured"
                    )
                    return False
                ...
                if not endpoint_url.startswith(
                    ...
                    print(
                        f"WARNING: REST endpoint URL should start with https:// or http://: {endpoint_url}"
                    )
                ...
                if timeout < 1 or timeout > 300:
                    print(
                        f"WARNING: REST timeout should be between 1-300 seconds, got {timeout}"
                    )
                ...
                if provider_id:
                    ...
                    if api_key:
                        print(
                            f"[REST] API key configured (via credential manager, provider: {provider_id})"
                        )
                    else:
                        print(
                            f"WARNING: [REST] Provider {provider_id} configured but API key not found in credential store"
                        )
                else:
                    ...
                    if api_key:
                        print(
                            "[REST] API key configured (via rest_api_key - deprecated, consider migrating)"
                        )
                ...
                if rest_headers and isinstance(rest_headers, dict):
                    header_count = len(
                        [
                            k
                            for k in rest_headers.keys()
                            if rest_headers.get(k) is not None
                        ]
                    )
                    if header_count > 0:
                ...
                if rest_body and isinstance(rest_body, dict):
                    body_count = len(
                        [k for k in rest_body.keys() if rest_body.get(k) is not None]
                    )
                    if body_count > 0:
                        print(
                            f"[REST] Custom body fields configured ({body_count} fields)"
                        )
            ...
            try:
                ...
                if not model_file.exists():
                    ...
                    print(
                        f"[ERROR] Download with: hyprwhspr model download {self.current_model}"
                    )
                    return False
                ...
                print(
                    f"[BACKEND] pywhispercpp ({gpu_backend}) - model: {self.current_model}"
                )
                self.ready = True
    ...
    def _detect_gpu_backend(self) -> str:
        ...
        backend_config = self.config.get_setting(
        ...
        if backend_config == "nvidia":
            ...
            if shutil.which("vulkaninfo"):
                try:
                    result = subprocess.run(
                        ["vulkaninfo", "--summary"], capture_output=True, timeout=2
                    )
        ...
        if shutil.which("rocm-smi") or os.path.exists("/opt/rocm"):
            try:
                result = subprocess.run(
                    ["rocm-smi", "--showproductname"], capture_output=True, timeout=2
                )
                if result.returncode == 0:
    ...
    def _get_websocket_url(
        self, provider_id: str, model_id: str, mode: str = "transcribe"
    ) -> str:
        """
        ...
        if "websocket_endpoint" in provider:
            ...
            if not endpoint:
                raise ValueError(
                    f"Provider {provider_id} has no endpoint or websocket_endpoint"
                )

            # Transform: https:// -> wss://, replace /audio/transcriptions -> /realtime
            base_url = endpoint.replace("https://", "wss://").replace(
                "http://", "ws://"
            )
    ...
    def _transcribe_rest(
        ...
        try:
            ...
            if not isinstance(rest_headers, dict):
                print(
                    "WARNING: rest_headers must be an object/dict; ignoring invalid value"
                )
                rest_headers = {}

            if not isinstance(rest_body, dict):
                print(
                    "WARNING: rest_body must be an object/dict; ignoring invalid value"
                )
                rest_body = {}
            ...
            for key, value in rest_headers.items():
                ...
                try:
                    ...
                    print(
                        f"WARNING: Skipping non-serializable rest_headers entry: {key}"
                    )
            ...
            for key, value in rest_body.items():
                ...
                try:
                    ...
                    print(
                        f"WARNING: Skipping rest_body entry with non-stringable key: {key}"
                    )
                    continue

                if isinstance(value, (dict, list, tuple, set)):
                    print(
                        f"WARNING: rest_body values must be scalar (key: {key_str}); skipping entry"
                    )
                    continue
            ...
            if data:
                ...
                param_summary = ", ".join(
                    f"{k}={v[:20] + '...' if isinstance(v, str) and len(v) > 20 else v}"
            ...
            response = requests.post(
                endpoint_url, files=files, data=data, headers=headers, timeout=timeout
            )
            ...
            try:
                ...
                print(
                    f"[REST] Content-Type: {response.headers.get('Content-Type', 'not set')}"
                )
                return ""
    ...
    def _reconnect_realtime_client(self) -> bool:
        ...
        api_key_header = params.get("api_key_header")

        if not (websocket_url and model_id):
            print(
                "[REALTIME] Missing connection parameters; cannot reconnect", flush=True
            )
            return False

        try:
            if self._is_loopback_websocket_url(websocket_url):
                if not self._ensure_local_ws_server_running(websocket_url):
                    return False
            ...
            try:
                connected = self._realtime_client.connect(
                    websocket_url,
                    api_key,
                    model_id,
                    instructions,
    ...
    def _is_loopback_websocket_url(self, websocket_url: str) -> bool:
        try:
    ...
    def _ensure_local_ws_server_running(self, websocket_url: str) -> bool:
        if not self._is_loopback_websocket_url(websocket_url):
            return True

        server_config = self.config.get_setting("local_model_server", {})
        if not isinstance(server_config, dict):
            server_config = {}
        if not bool(server_config.get("auto_start", True)):
            return True

        model_path = server_config.get("model_path")
        if not model_path:
            print(
                "ERROR: Local model server auto-start enabled but local_model_server.model_path is missing",
                flush=True,
            )
            return False
        ...
        try:
            ...
            restart_max = int(server_config.get("max_restarts", 3))
            if self._local_server_restart_attempts >= restart_max:
                print(
                    "ERROR: Local model server restart limit reached; using cloud backend is recommended",
                    flush=True,
                )
                return False

            self._local_server_restart_attempts += 1
            ...
            started = controller.start_server(
            ...
            if not started:
                print(
                    "ERROR: Failed to auto-start local model websocket server",
                    flush=True,
                )
                return False
            ...
            print(
                f"ERROR: Failed to ensure local model server is running: {e}",
                flush=True,
            )
            return False
    ...
    def transcribe_audio(
        ...
        try:
            ...
            if not isinstance(audio_data, np.ndarray):
                print(
                    f"Invalid audio data type: {type(audio_data)}, expected numpy.ndarray",
                    flush=True,
                )
                return ""
            ...
            if audio_data.ndim != 1:
                print(
                    f"Invalid audio data shape: {audio_data.shape}, expected 1D array",
                    flush=True,
                )
            ...
            if len(audio_data) < min_samples:
                print(
                    f"Audio too short: {len(audio_data)} samples (minimum {min_samples})",
                    flush=True,
                )
                return ""
            ...
            if rms < 1e-6:  # Extremely quiet
                print(
                    f"Audio level too low (RMS: {rms:.2e}) - likely invalid", flush=True
                )
                return ""
        ...
        if backend == "rest-api":
            ...
            if self._pywhisper_model is not None:
                print(
                    f"[WARN] REST API backend selected but pywhispercpp model is loaded - this should not happen",
                    flush=True,
                )
            return self._transcribe_rest(
                audio_data, sample_rate, language_override=language_override
            )
        ...
        with self._model_lock:
            ...
            if time_since_last_use > 300 and self._last_use_time > 0:
                print(
                    "[MODEL] Long idle period detected - reinitializing model (suspend/resume likely)",
                    flush=True,
                )
            ...
            try:
                ...
                with self._intercept_progress_logs():
                    ...
                    segments = self._pywhisper_model.transcribe(
    ...
    def _reinitialize_model(self) -> bool:
        ...
        print(
            "[MODEL] Reinitializing whisper model (suspend/resume detected)", flush=True
        )
        ...
        try:
            ...
            self._pywhisper_model = Model(
                model=model_name, n_threads=threads, redirect_whispercpp_logs_to=None
            )
    ...
    def set_threads(self, num_threads: int) -> bool:
        ...
        with self._model_lock:
            try:
                ...
                if self._pywhisper_model and hasattr(
                    self._pywhisper_model, "set_n_threads"
                ):
                    try:
    ...
    def set_model(self, model_name: str) -> bool:
        ...
        if backend == "rest-api":
            print(
                "ERROR: Cannot change model when using REST API backend - switch to pywhispercpp"
            )
        ...
        with self._model_lock:
            try:
                ...
                if not self._validate_model_file(model_name):
                    ...
                    print(
                        "Please download the model to ~/.local/share/pywhispercpp/models/"
                    )
                    return False
